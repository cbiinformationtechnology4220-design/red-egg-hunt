...
describe('Google Sheets backend source contract', () => {
  it('defines exactly 60 winning codes, locked submission, one-time claim, counters, and private audit storage', async () => {
    const script = await readFile('google-apps-script/Code.gs', 'utf8');
    expect(script).toContain('var TOTAL_CODES = 60;');
